...
- **Add / Remove, not overwrite** — one box per card: type a number and hit **+ Add** to log a new batch ("just made 15 lunches") on top of the running count, or **−** to take off meals lost to spoilage or waste. Neither replaces the count.
- **One-screen layout** — header, both cards and the controls fit a phone screen without scrolling; account and sync status live in a single header chip
...
2. When you cook a fresh batch of lunches or a dinner delivery arrives, type how many meals you got into the card's box and hit **+ Add** — it tops up the existing count rather than replacing it.
3. If any meals spoil or get wasted, type the amount into the same box and hit **−**.
4. Each card shows:
...
filled in, none of this runs** — the app stays pure-localStorage with no account
chip and no sign-in, exactly as before.
...
- **Signed out is read-only, not blocked.** With sync configured but nobody
  signed in, the board still shows this device's counts; the header chip goes
  amber ("Sign in to sync") and each card's **+ Add** / **−** controls are
  hidden until you sign in through the bottom sheet. Once you're in, that same
  chip shows sync state (syncing / synced / offline / error) and holds the
  **Sign out** menu — nothing is stranded at the bottom of the page.
...
Or straight from the command line (no Android Studio):
